...
import type { LaunchInfo } from "../../src/shared/store";
...
import BackendManager from "../../src/main/backend-manager";
...
describe("BackendManager", () => {
  ...
  test("should read backend ready timeout from env", async () => {
    ...
    process.env.CLOD_PET_BACKEND_READY_MAX_RETRIES = "90";
    process.env.CLOD_PET_BACKEND_READY_INTERVAL_MS = "750";

    const waitSpy = jest.spyOn(manager as any, "_waitForReady").mockResolvedValue("ok");

    try {
      await manager.start();
      expect(waitSpy).toHaveBeenCalledWith(90, 750);
    } finally {
      waitSpy.mockRestore();
      process.env.CLOD_PET_BACKEND_READY_MAX_RETRIES = originalRetries;
      process.env.CLOD_PET_BACKEND_READY_INTERVAL_MS = originalInterval;
    }
  });
  ...
  test("should start backend using source when explicitly requested", async () => {
    const fs = require("fs");
    (fs.existsSync as jest.Mock).mockReturnValue(true);
    const originalEnv = process.env.CLOD_PET_BACKEND_MODE;
    process.env.CLOD_PET_BACKEND_MODE = "source";
  ...
  test("should suppress backend debug api command stderr spam", async () => {
    ...
    if (stderrCallback) {
      stderrCallback(Buffer.from('{"time":"2026-04-30T22:14:38.203299671-06:00","level":"DEBUG","msg":"api command","command":"remove_pet"}\n'));
    }

    expect(manager.lastStderr).toContain('"level":"DEBUG"');
  ...
  test("should mark backend unavailable after unexpected post-start crash", async () => {
    ...
    if (exitCallback) exitCallback(137);

    const diag = manager.getDiagnostics();
    expect(diag.available).toBe(false);
    expect(diag.state).toBe("fatal");
    expect(diag.fatalError).toContain("137");
    expect(diag.lastError).toContain("137");
  });
