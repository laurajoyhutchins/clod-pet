...
class BorderDetector {
    ...
    checkGravity(x, y, width, height) {
        ...
        const overlappingDisplays = displays.filter(d => centerX >= d.bounds.x && centerX <= d.bounds.x + d.bounds.width);
        if (overlappingDisplays.length === 0)
            return false;
        // Find the current display based on the pet's center
        const centerY = y + height / 2;
        const currentDisplay = overlappingDisplays.find(d => centerY >= d.bounds.y && centerY <= d.bounds.y + d.bounds.height) || overlappingDisplays[0];
